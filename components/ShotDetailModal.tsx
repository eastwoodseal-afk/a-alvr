...
interface Shot {
  id: string; image_url: string; title?: string; description?: string; username?: string; user_id?: string; author?: string; likes_count?: number; views_count?: number; uoc_id?: string; uoc_username?: string; category_id?: number | null; // 🔧 Acepta null de Supabase
}
...
interface ModalProps {
  ...
  onShotUpdated?: (updatedShot: Shot) => void; // NUEVO: Para actualizar el muro al editar
  onDisapprove?: (shotId: string) => void; // NUEVO: Para archivar desde el modal
}
...
export default function ShotDetailModal({ shot, onClose, isSaved, isSaving, onSave, user, isLiked, likesCount, onLike, viewsCount, onView, onOpenCollection, onShotUpdated, onDisapprove }: ModalProps) {
  ...
  // 🆕 FUNCIÓN DE GUARDADO DE CURADURÍA
  const handleCurateSave = async () => {
    ...
    if (error) {
      ...
      // Avisar al padre (HomeView) que el shot cambió para actualizar la tarjeta
      if (onShotUpdated) onShotUpdated({ ...shot, ...updateData });
      setCurateMode(false); // Salir de modo curaduría
    }
  ...
  return (
    <div className="fixed left-0 right-0 bottom-0 z-[49] flex flex-col items-center pt-3 pb-[20px]" style={{ top: '56px', background: 'rgba(10, 24, 51, 0.85)' }}>
        ...
        <div className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col gap-2 items-center z-10 bg-gray-500/50 backdrop-blur-sm rounded-xl p-2 mr-1">
          ...
          {/* 🆕 BOTÓN MODO CURADURÍA (SOLO ADMINS) */}
          {isAdmin && !curateMode && (
            <button 
              onClick={() => setCurateMode(true)} 
              className="bg-blue-600 hover:bg-blue-500 text-white rounded-full p-1.5 shadow transition" 
              title="Modo Curaduría"
            >
          ...
          {isAdmin && curateMode && (
             <button 
              onClick={() => { setCurateMode(false); setEditError(""); }} 
              className="bg-gray-600 hover:bg-gray-500 text-white rounded-full p-1.5 shadow transition" 
              title="Salir de Curaduría"
            >
          ...
          {/* 🆕 BOTÓN ARCHIVAR (SOLO ADMINS EN MODO CURADURÍA) */}
          {isAdmin && curateMode && onDisapprove && (
            <button 
              onClick={() => onDisapprove(shot.id)} 
              className="bg-orange-600 hover:bg-orange-500 text-white rounded-full p-1.5 shadow transition" 
              title="Archivar Shot"
            >
          ...
          {user && !curateMode && (
            <button
              ...
              style={isSaved ? { background: '#facc15', color: '#fff' } : { background: 'rgba(236, 72, 153, 0.35)', color: '#fff' }}
        ...
        <div className="w-full text-left">
          ...
          {/* 🆕 MODO CURADURÍA: CAMPOS EDITABLES */}
          {curateMode ? (
            <div className="space-y-3 mb-4">
              <div>
                <label className="text-[10px] text-gray-400 block mb-1">Título</label>
                <input type="text" className="w-full px-3 py-2 rounded-lg bg-gray-800 border border-gray-600 text-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" value={editTitle} onChange={e => setEditTitle(e.target.value)} disabled={editLoading} />
              </div>
              <div>
                <label className="text-[10px] text-gray-400 block mb-1">Arquitecto / Estudio</label>
                <input type="text" className="w-full px-3 py-2 rounded-lg bg-gray-800 border border-gray-600 text-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" value={editAuthor} onChange={e => setEditAuthor(e.target.value)} disabled={editLoading} />
              </div>
              <div>
                <label className="text-[10px] text-gray-400 block mb-1">Descripción</label>
                <textarea className="w-full px-3 py-2 rounded-lg bg-gray-800 border border-gray-600 text-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" rows={3} value={editDescription} onChange={e => setEditDescription(e.target.value)} disabled={editLoading} />
              </div>
              <div>
                <label className="text-[10px] text-gray-400 block mb-1">Categoría</label>
                <select className="w-full px-3 py-2 rounded-lg bg-gray-800 border border-gray-600 text-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" value={editCategoryId} onChange={e => setEditCategoryId(e.target.value)} disabled={editLoading}>
              ...
              <div className="flex gap-2">
                <button onClick={handleCurateSave} disabled={editLoading} className="flex-1 bg-green-600 hover:bg-green-500 text-white font-bold py-2 rounded-lg text-sm transition disabled:opacity-50">
                  {editLoading ? "Guardando..." : "Guardar Cambios"}
                </button>
                <button onClick={() => setCurateMode(false)} disabled={editLoading} className="flex-1 bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 rounded-lg text-sm transition">
                  Cancelar
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-800 gap-4">
              ...
              {shot.uoc_username ? (
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  {uocActive && onOpenCollection ? (
                    <button onClick={() => onOpenCollection(shot.uoc_id!)} className="text-[10px] text-blue-500/70 hover:text-blue-400 italic truncate hover:underline text-left">
                      @{shot.uoc_username}
                    </button>
                  ) : (
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <div className="w-6 h-6 rounded-full bg-gray-700 overflow-hidden flex items-center justify-center text-[9px] font-bold text-white border border-gray-600 flex-shrink-0">
                   {authorData?.avatar_url && !imgError ? (
                     <img src={authorData.avatar_url} className="w-full h-full object-cover" alt="Avatar" onError={() => setImgError(true)} /> 
                   ) : (
                ...
                {isGhost && (
                  <span className="text-[9px] bg-yellow-500/10 text-yellow-500 border border-yellow-500/30 px-2 py-0.5 rounded font-semibold">Pieza del Archivo</span>
                )}
