...
export default function ShareStudioModal({ open, onClose }: Props) {
  ...
  // 🆕 CIRUGÍA: Estado de confirmación para revocar
  const [confirmRevokeId, setConfirmRevokeId] = useState<string | null>(null);

  useEffect(() => {
    if (open && user?.id) fetchSharedUsers();
    if (!open) { setQuery(""); setSearchResults([]); setSuccessMsg(""); setConfirmRevokeId(null); }
  }, [open, user?.id]);

  const fetchSharedUsers = async () => {
    if (!user) return;
    setLoadingShared(true);
    const { data, error } = await supabase.from('studio_shares').select('viewer_id, profiles!studio_shares_viewer_id_fkey(id, username, avatar_url)').eq('owner_id', user.id);
    if (!error && data) {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    setSuccessMsg("");
    const { data, error } = await supabase.from('profiles').select('id, username, avatar_url').neq('id', user.id).ilike('username', `%${query}%`).limit(5);
  ...
  const handleShare = async (viewerId: string, viewerUsername: string) => {
    if (!user) return;
    const { error } = await supabase.from('studio_shares').insert({ owner_id: user.id, viewer_id: viewerId });
    if (error) {
      ...
      await supabase.from('notifications').insert({ user_id: viewerId, title: '🏗️ Acceso al Estudio', message: 'Te han compartido su Estudio de trabajo.', type: 'studio_share', read: false });
      fetchSharedUsers();
    }
  };

  // 🆕 CIRUGÍA: Revocación directa, sin confirm() nativo
  const handleRevoke = async (viewerId: string) => {
    if (!user) return;
    setRevokingId(viewerId);
    const { error } = await supabase.from('studio_shares').delete().match({ owner_id: user.id, viewer_id: viewerId });
    if (!error) {
        ...
        <div className="p-4 space-y-4">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-1">
                {sharedUsers.map(u => (
                  <div key={u.id} className="flex items-center justify-between bg-gray-800 p-2 rounded-lg">
                    ...
                    {/* 🆕 CIRUGÍA: Reemplazo del window.confirm() */}
                    {confirmRevokeId === u.id ? (
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] text-red-400 font-bold">¿Quitar?</span>
                        <button onClick={() => handleRevoke(u.id)} disabled={revokingId === u.id} className="text-xs bg-red-600 hover:bg-red-500 text-white font-bold px-2 py-0.5 rounded transition disabled:opacity-50">Sí</button>
                        <button onClick={() => setConfirmRevokeId(null)} className="text-xs bg-gray-600 hover:bg-gray-500 text-white font-bold px-2 py-0.5 rounded transition">No</button>
                      </div>
                    ) : (
                      <button 
          ...
          <div>
            ...
            <div className="flex gap-2 mb-3">
              <input type="text" placeholder="Buscar usuario..." value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} className="flex-1 bg-gray-800 border border-gray-700 rounded px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-yellow-500" />
              <button onClick={handleSearch} disabled={searching} className="px-4 bg-yellow-500 text-black font-bold rounded text-sm disabled:opacity-50">Buscar</button>
            </div>
            ...
            <div className="space-y-2">
              {searchResults.map(u => (
                <div key={u.id} className="flex items-center justify-between bg-gray-800 p-2 rounded-lg">
                  ...
                  <button onClick={() => handleShare(u.id, u.username)} className="text-xs text-yellow-400 hover:underline font-bold">Compartir</button>
                </div>
              ))}
            </div>
          </div>
