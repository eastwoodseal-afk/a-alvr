...
interface Shot {
  id: string; image_url: string; title?: string; description?: string; author?: string; username?: string;
}
...
interface Props {
  ...
  onReject: (id: string, reason: string) => void; // NUEVO: Ahora recibe la razón
}
...
export default function AdminShotDetailModal({ shot, onClose, onApprove, onReject }: Props) {
  ...
  // NUEVO: Estado para la razón de archivo
      ...
      <div className="relative bg-gray-900 rounded-2xl shadow-2xl text-gray-100 flex flex-col max-w-2xl w-full max-h-[90vh] border border-gray-700" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:flex md:gap-6">
          ...
          <div className="md:w-1/2 space-y-3">
            ...
            <div>
              <label className="text-xs text-gray-400 block mb-1">Asignar Categoría:</label>
              {!isAdding ? (
        ...
        <div className="p-4 border-t border-gray-700 flex flex-col gap-3 flex-shrink-0">
          ...
          {/* NUEVO: Celda de razón de archivo */}
          <div className="flex items-center gap-3">
            <input 
              type="text" 
              placeholder="Razón de archivo (opcional, visible para el usuario)..." 
              value={rejectionReason} 
              onChange={(e) => setRejectionReason(e.target.value)} 
              className="flex-1 px-3 py-2 rounded-lg bg-gray-800 border border-gray-600 text-white text-sm focus:outline-none focus:ring-1 focus:ring-orange-500"
            />
            <button 
              onClick={() => onReject(shot.id, rejectionReason)} 
              disabled={editLoading} 
              className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-lg transition disabled:opacity-50 text-sm whitespace-nowrap"
            >
              Archivar
            </button>
          </div>
