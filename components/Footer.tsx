...
// 🆕 INTERFAZ PARA EVITAR EL ERROR DE 'unknown'
interface TagItem {
  id: number;
  name: string;
  slug: string;
...
export default function Footer() {
  ...
  // 🆕 TIPADO EXPLÍCITO PARA LOS TAGS
  ...
  useEffect(() => {
    setLoadingTags(true);
    supabase.from('tags').select('id, name, slug, facet').order('facet').order('name').then(({ data }) => {
      if (data) setTags(data as TagItem[]); // 🆕 Castear respuesta
      ...
      <footer className="fixed bottom-0 left-0 right-0 w-full h-16 py-4 px-6 bg-gray-800 text-gray-200 text-sm flex items-center justify-between z-40 border-t border-gray-700">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button id="category-btn" className={`rounded-full w-[28px] h-[28px] flex items-center justify-center shadow transition-all ${!categoryFilterEnabled ? 'bg-gray-700/50 text-gray-600 cursor-not-allowed opacity-50' : selectedCategory ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`} onClick={() => categoryFilterEnabled && setShowCategoryDropdown(!showCategoryDropdown)} disabled={!categoryFilterEnabled} title={categoryFilterEnabled ? "Filtrar por categoría" : "Filtro desactivado"}>
          ...
          <div className="relative">
            <button id="tag-btn" className={`rounded-full w-[28px] h-[28px] flex items-center justify-center shadow transition-all ${selectedTag ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`} onClick={() => setShowTagDropdown(!showTagDropdown)} title="Filtrar por etiqueta">
          ...
          {/* BOTÓN TOGGLE VISTA */}
          <button onClick={handleToggleView} className={`rounded-full w-[28px] h-[28px] flex items-center justify-center shadow transition-all ${isSingleColumn ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`} title={isSingleColumn ? "Vista Mosaico" : "Vista Feed"}>
            {isSingleColumn ? (
