...
export default function Header() {
  ...
  // Evaluamos si hay algo abierto para cambiar el icono del avatar a una X
  const anythingOpen = showMyShots || showSavedShots || showAdmin || showAbout || showFollowing || showMenu || showLoginModal || showConfig; // ACTUALIZADO

  // Función maestra para cerrar todo
  const closeAllOverlays = () => {
    ...
    setShowConfig(false); // ACTUALIZADO
  };
  ...
  // ACTUALIZADO: 'profile' reemplazado por 'config'
  const handleMenuNavigate = (action: 'shots' | 'config' | 'saved' | 'collection') => {
    closeAllOverlays();
    if (action === 'shots') setShowMyShots(true);
    if (action === 'saved') { setSavedShotsInitialView(null); setShowSavedShots(true); } // Bodega
    if (action === 'collection') { setSavedShotsInitialView('all'); setShowSavedShots(true); } // Vitrina
    if (action === 'config') setShowConfig(true); // ACTUALIZADO
  };
    ...
    <>
      <header className="fixed top-0 left-0 w-full h-14 flex items-center justify-between px-6 bg-gray-900 shadow z-50">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          {/* MANIFIESTO */}
          <button 
            onClick={() => { closeAllOverlays(); setShowAbout(!showAbout); }} 
            className={`rounded-full h-7 w-7 flex items-center justify-center transition-all duration-200 border ${showAbout ? 'bg-yellow-500 text-black border-yellow-500' : 'bg-transparent text-gray-500 hover:bg-gray-700 hover:text-white border-gray-700'}`} 
            title="Manifiesto"
          >
          ...
          {session && (
            <button 
              onClick={() => { closeAllOverlays(); setShowFollowing(!showFollowing); }} 
              className={`rounded-full h-7 w-7 flex items-center justify-center border transition-all duration-200 ${showFollowing ? 'bg-yellow-500 border-yellow-400 text-black' : 'bg-gray-700 border-gray-600 text-gray-200 hover:bg-gray-600'}`} 
              title="Relaciones"
            >
          ...
          {loading ? (
            ...
            <button 
              id="user-menu-btn"
              className={`rounded-full h-7 w-7 flex items-center justify-center border focus:outline-none relative overflow-hidden transition-all duration-200 p-0 ring-2 ring-transparent hover:ring-purple-500 ${anythingOpen ? 'bg-red-500 hover:bg-red-600 border-red-400' : 'bg-yellow-500 border-gray-200'}`} 
              onClick={() => { if (anythingOpen) closeAllOverlays(); else setShowMenu(!showMenu); }}
            >
              {anythingOpen ? (
      ...
      {/* ACTUALIZADO: OVERLAY DE CONFIGURACIÓN */}
