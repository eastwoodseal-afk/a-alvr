...
export default function PublicCollectionOverlay({ userId, onClose }: Props) {
  ...
  useEffect(() => {
    const fetchShots = async () => {
      ...
      if (viewMode === 'all') {
        ...
        created.forEach(s => shotMap.set(s.id, s)); 
        saved.forEach(s => { if (!shotMap.has(s.id)) shotMap.set(s.id, s); else { 
  ...
  return (
    <div className="fixed top-14 left-0 right-0 bottom-0 z-[50] bg-gray-950 flex flex-col"> {/* 🆕 Z-50 ESTANDARIZADO */}
      ...
      <div className="flex-1 flex flex-col relative" style={{ background: 'rgba(20,20,20,0.95)' }}>
        ...
        <div className="w-full flex items-center justify-between px-4 py-3 sticky top-0 bg-[rgba(20,20,20,0.95)] z-10 border-b border-yellow-500">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <button onClick={() => viewMode === 'all' ? onClose() : setViewMode('all')} className="w-7 h-7 rounded-full bg-gray-700 hover:bg-gray-600 text-yellow-500 flex items-center justify-center flex-shrink-0 transition mt-0.5">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2" style={{ maxHeight: 'calc(100vh - 120px)' }}>
          {loading ? <div className="text-center py-8 text-gray-400 text-xs animate-pulse">Cargando kolección...</div> :
      ...
      {/* 🆕 MODAL DETALLE CORREGIDO (Amnesia curada + UOC conectado) */}
