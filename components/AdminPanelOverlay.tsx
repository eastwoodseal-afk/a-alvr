...
import AdminSettings from "./AdminSettings"; // NUEVO
...
export default function AdminPanelOverlay({ userId, userRole, onClose }: Props) {
  const [activeTab, setActiveTab] = useState<'shots' | 'users' | 'categories' | 'settings'>('shots'); // ACTUALIZADO
    ...
    <div className="fixed top-14 left-0 right-0 bottom-0 z-[60] bg-gray-950 flex flex-col md:flex-row overflow-hidden">
      ...
      <aside className="w-full md:w-48 bg-gray-900/95 backdrop-blur-sm border-r border-gray-700 flex-shrink-0 flex flex-col overflow-y-auto custom-scrollbar">
        ...
        <div className="flex md:flex-col gap-1 p-2 overflow-x-auto md:overflow-x-hidden">
          ...
          {isSuperAdmin && (
            <TabButton active={activeTab === 'users'} onClick={() => setActiveTab('users')} icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" /></svg>} label="Usuarios" />
          )}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative flex flex-col">
        <div className="p-3 flex-1">
          ...
          {activeTab === 'settings' && <AdminSettings />} {/* NUEVO */}
        </div>
      </div>
    </div>
  );
