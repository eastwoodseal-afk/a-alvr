...
import AdminFacetManager from "./AdminFacetManager"; // 🆕 IMPORT
...
export default function AdminPanelOverlay({ userId, userRole, onClose }: Props) {
  // 🆕 Añadido 'facets' a los tipos de pestaña
  const [activeTab, setActiveTab] = useState<'shots' | 'users' | 'categories' | 'tags' | 'facets' | 'settings'>('shots');
    ...
    <div className="fixed top-14 left-0 right-0 bottom-0 z-[60] bg-gray-950 flex flex-col md:flex-row overflow-hidden">
      ...
      <aside className="w-full md:w-48 bg-gray-900/95 backdrop-blur-sm border-r border-gray-700 flex-shrink-0 flex flex-col overflow-y-auto custom-scrollbar">
        ...
        <div className="flex md:flex-col gap-1 p-2 overflow-x-auto md:overflow-x-hidden">
          ...
          {/* 🆕 TAB FACETAS */}
          <TabButton active={activeTab === 'facets'} onClick={() => setActiveTab('facets')} icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" /></svg>} label="Facetas" />
        ...
        {/* ENLACE CUARENTENA (Aislado del grupo de tabs) */}
        {isSuperAdmin && (
          <div className="mt-auto border-t border-gray-700 p-2">
            <a 
              ...
              className="w-full py-1.5 px-2 rounded font-semibold text-left text-[10px] transition flex items-center gap-2 text-gray-600 hover:text-red-400 hover:bg-gray-800"
            >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative flex flex-col">
        <div className="p-3 flex-1">
          ...
          {activeTab === 'tags' && <AdminTagManager />}
          {activeTab === 'facets' && <AdminFacetManager />} {/* 🆕 RENDERIZADO */}
