...
export default function MyShotsOverlay({ userId, onClose }: Props) {
  ...
  const handleLinkObra = async () => {
    if (!obraName.trim() || selectedShots.length === 0) return;
    setLinkingObra(true);
    ...
    if (success) {
      ...
      alert("Error al vincular la obra.");
    }
  ...
  return (
    <div className="fixed top-14 right-0 bottom-0 left-0 z-[50] bg-gray-950 flex flex-col">
      ...
      <div className="flex-shrink-0 flex items-center px-4 py-3 border-b border-yellow-500 bg-[#0a1833] gap-2">
        ...
        <h2 className="text-base font-semibold text-gray-200 flex-shrink-0">
          ...
          {isObraCellActive && (
            <div className="flex items-center gap-1 flex-shrink-0">
              <button 
                ...
                disabled={!obraName.trim() || linkingObra}
                className="w-5 h-5 rounded-full bg-green-600 hover:bg-green-500 text-white flex items-center justify-center transition disabled:opacity-50"
                title="Confirmar"
              >
              ...
              <button 
                onClick={() => { setSelectedShots([]); setObraName(""); }}
                className="w-5 h-5 rounded-full bg-gray-600 hover:bg-gray-500 text-white flex items-center justify-center transition"
                title="Cancelar"
              >
