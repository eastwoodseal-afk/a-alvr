...
export default function MyShotsOverlay({ userId, onClose }: Props) {
  ...
  const handleRelinquishShot = (shotId: string) => {
   ...
   // 🆕 FUNCIÓN: GENERAR INVITACIÓN Y ABRIR WHATSAPP (MEJORADO)
  const handleWhatsAppShare = async () => {
    ...
    try {
      ...
      // 🆕 FORMATO LIMPIO: URL en su propia línea para que WhatsApp la detecte como enlace
      const text = `¡Hola! Te invito a ver mi Estudio de Arquitectura en A'AL VR 🏛️\n\n👉 ${url}`;
      
      // 🆕 Usar api.whatsapp.com que es más confiable para compartir links
  ...
  return (
    <div className="fixed top-14 right-0 bottom-0 left-0 z-[60] bg-gray-950 flex flex-col">
      ...
      <div className="flex-shrink-0 flex items-center px-4 py-3 border-b border-yellow-500 bg-[#0a1833]">
        ...
        {/* Botón Compartir Estudio (Existente) */}
      ...
      {selectedShot && ( <MyShotDetailModal shot={selectedShot} user={user} onClose={() => setSelectedShot(null)} onUpdate={handleUpdateShot} onRelinquish={handleRelinquishShot} /> )}
      <ShareStudioModal open={showShareModal} onClose={() => setShowShareModal(false)} />
    </div>
  );
}