    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-[80]" onClick={onClose}>
      <div className="bg-gray-900 rounded-xl w-full max-w-sm border border-gray-700 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          <div>
            <label className="text-xs text-gray-400 block mb-1">Razón del archivo (opcional):</label>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button type="submit" disabled={loading} className="px-4 py-2 rounded-lg bg-orange-600 hover:bg-orange-700 text-white text-sm font-bold transition disabled:opacity-50">{loading ? "Archivando..." : "Archivar"}</button>
          </div>
        </form>
      </div>
    </div>
