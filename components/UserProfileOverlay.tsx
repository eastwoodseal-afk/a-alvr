...
export default function UserProfileOverlay({ userId, onClose, studioMode = false }: Props) {
  ...
  const [selectedCollectionId, setSelectedCollectionId] = useState<string | null>(null); // 🆕 UOC

  useEffect(() => {
    const fetchProfile = async () => { const { data } = await supabase.from('profiles').select('*').eq('id', userId).single(); if (data) setProfile(data); };
    const fetchShots = async () => {
      setLoading(true);
      let data = null, error = null;
      if (studioMode) { const r = await supabase.rpc('get_studio_shots', { p_owner_id: userId }); data = r.data; error = r.error; } 
      else { const r = await supabase.from('shots').select('id, image_url, title, description, user_id, author, likes_count, views_count, profiles!shots_user_id_fkey(username)').eq('user_id', userId).eq('is_approved', true).order('created_at', { ascending: false }); data = r.data; error = r.error; }
      if (error) { setShots([]); } else if (data) { setShots(data.map((s: any) => ({...s, id: s.id.toString()}))); }
      setLoading(false);
    };
    ...
    const fetchUserInteractions = async () => {
      ...
      const [savedRes, likedRes] = await Promise.all([ supabase.from("saved_shots").select("shot_id").eq("user_id", currentUser.id), supabase.from("likes").select("shot_id").eq("user_id", currentUser.id) ]);
  ...
  const handleToggleFollow = async () => {
    if (!currentUser || followLoading) return; setFollowLoading(true);
    const { data } = await supabase.rpc('toggle_follow', { target_user_id: userId });
    setIsFollowing(data); setProfile((prev: any) => ({ ...prev, followers_count: (prev.followers_count || 0) + (data ? 1 : -1) })); setFollowLoading(false);
  };
  ...
  const handleLike = async (shotId: string) => { 
    if (!currentUser) return; const alreadyLiked = likedShots.includes(shotId); 
    ...
    <div className="fixed top-14 right-0 bottom-0 left-0 z-[50] bg-gray-950 flex flex-col"> {/* 🆕 Z-50 */}
      ...
      <div className="flex items-center gap-2 px-2 py-1 border-b border-gray-800 bg-gray-900/50 flex-shrink-0 h-10">
        ...
        <div className="flex items-center gap-1.5 flex-1 min-w-0 h-7 overflow-hidden">
            ...
            {isInactive && (<span className="text-[8px] bg-red-900/50 text-red-400 border border-red-800 px-1.5 rounded font-bold flex-shrink-0 uppercase tracking-wider">Inactivo</span>)}
            ...
            {!isInactive && (<div className="hidden sm:flex items-center gap-2 text-[9px] text-gray-400 border-l border-gray-700 pl-1.5 ml-1 flex-shrink-0"><span><strong className="text-white">{profile?.followers_count || 0}</strong> Seg</span><span><strong className="text-white">{shots.length}</strong> Shots</span></div>)}
        </div>
      ...
      {isInactive ? (
        ...
        <div className="flex-1 overflow-y-auto p-1 custom-scrollbar">
          {loading ? <div className="text-center py-8 text-gray-400">Cargando...</div> : shots.length === 0 ? <div className="text-center py-8 text-gray-600">{studioMode ? "El estudio está vacío." : "Sin shots."}</div> :
           <div className="columns-2 md:columns-3 lg:columns-4 xl:columns-6 gap-2 w-full">
              {shots.map(shot => (<ShotCard key={shot.id} shot={shot} isSaved={savedShots.includes(shot.id)} isSaving={false} onSave={() => handleSave(shot.id)} isLiked={likedShots.includes(shot.id)} likesCount={shot.likes_count || 0} isLiking={false} onLike={() => handleLike(shot.id)} viewsCount={shot.views_count || 0} user={currentUser} onClick={() => setSelectedShot(shot)} />))}
           </div>
          }
        </div>
      )}
      ...
      {!isInactive && selectedShot && ( 
        <ShotDetailModal 
          ...
          onSaveChange={(newIsSaved) => { if(newIsSaved) setSavedShots(prev => [...prev, selectedShot.id]); }}
