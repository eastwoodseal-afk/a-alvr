...
import { supabase } from "../lib/supabaseClient"; // RUTA CORREGIDA PARA LAB
import { useAuth } from "../lib/AuthContext"; // RUTA CORREGIDA PARA LAB
...
export default function NotificationBell({ open, onClose }: Props) {
  ...
  const [loading, setLoading] = useState(false);
    ...
    useEffect(() => {
    if (!user?.id) return; // Seguro: no suscribir si no hay ID
    
    fetchNotifications();
    const channel = supabase.channel(`realtime-notifications-${user.id}`) // Canal único por ID
  ...
  }, [user?.id]); // <--- LA CURA: Solo depende del ID
