...
export default function ShotCard({ shot, isSaved, isSaving, onSave, isLiked, likesCount, isLiking, onLike, viewsCount, user, onClick, hideViews, hideSave, boardName, isAdmin, onDisapprove, isDisapproving }: Props) {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {!hideViews && (
              <div className="flex flex-col items-center justify-center">
                ...
                <span className="text-[10px] text-gray-500 font-mono">{viewsCount || 0}</span>
              </div>
            )}

            <div className="flex flex-col items-center justify-center">
              <button onClick={(e) => { e.stopPropagation(); onLike(); }} disabled={!user || isLiking} className={`transition-transform active:scale-125 ${isLiking ? 'animate-pulse' : ''}`}>
                <svg xmlns="http://www.w3.org/2000/svg" fill={isLiked ? "#ef4444" : "none"} viewBox="0 0 24 24" strokeWidth={2} stroke={isLiked ? "#ef4444" : "#9ca3af"} className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>
              </button>
      ...
      {/* --- BOTÓN DESAPROBAR (IZQUIERDA - SOLO ADMINS) --- */}
