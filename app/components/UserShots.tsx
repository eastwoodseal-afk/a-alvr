...
export default function UserShots({ userId }: { userId: string }) {
  ...
  const fetchUserShots = useCallback(async (pageNum: number) => {
    ...
    const { data, error } = await supabase
      .from("shots")
      .select("id, image_url, title, description, user_id, author, likes_count")
      .eq("user_id", userId)
      .order("id", { ascending: false })
      .range(start, end);

    if (error) { console.error("Error:", error); setLoading(false); setInitialLoad(false); return; }
  ...
  const loadMore = useCallback(() => {
    if (hasMore && !loading) { const nextPage = page + 1; setPage(nextPage); fetchUserShots(nextPage); }
