...
export default function ShotDetailModal({ shot, onClose, isSaved, isSaving, onSave, user, isLiked, likesCount, onLike, viewsCount, onView }: ModalProps) {
  ...
  const fetchAuthorData = async () => {
    ...
    try {
      const { data, error } = await supabase.from('profiles').select('followers_count, avatar_url').eq('id', shot.user_id).single();
      if (error) throw error;
      if (data) setAuthorData({ followers_count: data.followers_count || 0, avatar_url: data.avatar_url || null });
    } catch (err) { setAuthorData({ followers_count: 0 }); }
  ...
  useEffect(() => { fetchAuthorData(); }, [shot?.user_id, currentUser]);
  ...
  return (
    // 1. Overlay: pt-3 (espacio header), sin scroll basura
    <div className="fixed left-0 right-0 bottom-0 z-[49] flex flex-col items-center pt-3 pb-[20px]" style={{ top: '56px', background: 'rgba(10, 24, 51, 0.85)' }}>
        ...
        {/* 2. Tarjeta: max-h-[90vh] (no se sale de pantalla), scroll interno, padding reducido p-4 */}
        <div
        ...
        {/* 3. Botones: right-0 (pegados al borde) */}
        <div className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col gap-2 items-center z-10 bg-gray-500/50 backdrop-blur-sm rounded-xl p-2 mr-1">
          ...
          {/* 4. Botón Cerrar: Icono SVG centrado con padding (p-1.5) */}
          <button 
            onClick={onClose} 
            className="bg-yellow-500 hover:bg-yellow-600 text-black rounded-full p-1.5 shadow transition"
            aria-label="Cerrar"
          >
        ...
        {/* IMAGEN: Sin max-height, tamaño natural */}
        <img 
            src={shot.image_url} 
            alt={shot.title || "Shot"} 
            ...
            onLoad={e => {
                ...
                if (img.parentElement) {
                  if (window.innerWidth >= 640) {
                    // Math.max elige el MAYOR de los dos
const naturalWidth = window.innerWidth * 0.65;
